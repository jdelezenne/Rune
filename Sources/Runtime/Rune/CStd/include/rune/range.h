...
typedef struct RuneRangeIterator
{
    ...
    int64 last_value;         ///< Value most recently yielded by `rune_range_iterator_next`;
                               ///< `current` has already been advanced past it by then, so
                               ///< callers that need a stable pointer to the yielded value
                               ///< (see `rune_runtime_range_iterator_next`) read this instead.
} RuneRangeIterator;
...
/// @return The count of elements as a pointer-sized integer (matches Rune `Int`),
///         or 0 if the range pointer is null or infinite.
intptr rune_range_count(RuneRange const* range);
...
/// @return Element count as a pointer-sized integer (matches Rune `Int`), or 0 when
///         range is null or empty.
intptr rune_range_inclusive_count(RuneRange const* range);
...
/// @return The element value at the given index as a pointer-sized integer (matches
///         Rune `Int` / `Bound` for integer ranges), or 0 if index is out of bounds
///         or range is null.
intptr rune_range_get(RuneRange const* range, intptr index);

/// Returns the integer element at zero-based offset `index` in the inclusive range.
/// @param range Pointer to the range.
/// @param index Zero-based index of the element to retrieve.
/// @return Element value at the given index as a pointer-sized integer, or 0 when
///         out of bounds or range is null.
intptr rune_range_inclusive_get(RuneRange const* range, intptr index);
