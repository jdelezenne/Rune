...
/// @param self Target sorted map.
void rune_sortedmap_remove_all(RuneSortedMap* self);
...
/// In-order cursor over the key/value pairs of a `RuneSortedMap`.
typedef struct RuneSortedMapIterator
{
    RuneSortedMap const* map; ///< Map being iterated.
    uint8 const* currentKey;  ///< Key bytes most recently yielded, or null before the first call.
    bool started;             ///< Whether `rune_sortedmap_iterator_next_key` has been called yet.
} RuneSortedMapIterator;

/// Creates a cursor positioned before the smallest key of `map`.
/// @param out Uninitialised iterator storage.
/// @param map Sorted map to iterate.
void rune_sortedmap_iterator_create(RuneSortedMapIterator* out, RuneSortedMap const* map);

/// Advances to the next key in ascending order and returns a pointer to its
/// bytes, or null when exhausted. Call `rune_sortedmap_iterator_current_value`
/// afterward to read the paired value for the same key.
/// @param self Iterator to advance.
/// @return Read-only pointer to the key bytes, or null when exhausted.
void const* rune_sortedmap_iterator_next_key(RuneSortedMapIterator* self);

/// Returns a pointer to the value bytes for the key most recently yielded by
/// `rune_sortedmap_iterator_next_key`. Undefined when called before the
/// first `next_key` call or after `next_key` returned null.
/// @param self Iterator positioned on a key.
/// @return Read-only pointer to the value bytes.
void const* rune_sortedmap_iterator_current_value(RuneSortedMapIterator* self);
