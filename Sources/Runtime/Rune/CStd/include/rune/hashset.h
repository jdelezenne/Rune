...
/// Unordered cursor over the occupied slots of a `RuneHashSet`. Iteration
/// order is unspecified and invalidated by any insert/remove on the set.
typedef struct RuneHashSetIterator
{
    RuneHashSet const* set; ///< Set being iterated.
    usize index;            ///< Next slot index to examine.
} RuneHashSetIterator;

/// Creates a cursor positioned before the first occupied slot of `set`.
/// @param out Uninitialised iterator storage.
/// @param set Hash set to iterate.
void rune_hashset_iterator_create(RuneHashSetIterator* out, RuneHashSet const* set);

/// Advances to the next occupied slot and returns a pointer to its element
/// bytes, or null when exhausted.
/// @param self Iterator to advance.
/// @return Read-only pointer to the element bytes, or null when exhausted.
void const* rune_hashset_iterator_next(RuneHashSetIterator* self);
