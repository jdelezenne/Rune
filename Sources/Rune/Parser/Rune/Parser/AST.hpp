...
namespace rune::parser
{
    ...
    enum class ASTNodeKind
    {
        ...
        EvalExpression,      ///< Compile-time evaluation expression (`eval expr`).
        DefaultExpression,   ///< Contextual default value expression (`default`).
        UninitExpression,    ///< Contextual uninitialized value expression (`uninit`).
                             /// @}
    ...
    /// Contextual default value expression node.
    class DefaultExpression final : public Expression
    {
    public:
        lexer::Token defaultToken; ///< `default` keyword token.

        explicit DefaultExpression(lexer::Token keyword)
            : defaultToken(std::move(keyword))
        {
            setLocation(defaultToken.location);
        }

        ASTNodeKind getKind() const override
        {
            return ASTNodeKind::DefaultExpression;
        }

        void accept(ASTVisitor& visitor) override;
    };

    /// Contextual uninitialized value expression node.
    class UninitExpression final : public Expression
    {
    public:
        lexer::Token uninitToken; ///< `uninit` keyword token.

        explicit UninitExpression(lexer::Token keyword)
            : uninitToken(std::move(keyword))
        {
            setLocation(uninitToken.location);
        }

        ASTNodeKind getKind() const override
        {
            return ASTNodeKind::UninitExpression;
        }
    ...
    public:
        ...
        SourceLocation operatorLocation;
        bool isPrefix{true};
        /// True when this reference-taking expression (`&expr`/`*expr`) was written as
        /// `escape &expr`/`escape *expr`, asserting the resulting reference may leave its
        /// creating scope. Only meaningful when op is ImmutableReference or MutableReference.
        bool isEscape{false};
    ...
    public:
        ...
        Expression* firstIndex()
        {
            return arguments.empty() ? nullptr : arguments.front().value.get();
        }

        Expression const* firstIndex() const
        {
            return arguments.empty() ? nullptr : arguments.front().value.get();
        }
    ...
    class EvalExpression final : public Expression
    {
    public:
        UniquePtr<Expression> expression;
        lexer::Token evalToken;

        EvalExpression(lexer::Token evalKeyword, UniquePtr<Expression> expr)
            : expression(std::move(expr))
            , evalToken(std::move(evalKeyword))
        {
            setLocation(this->evalToken.location);
        }

        ASTNodeKind getKind() const override
        {
            return ASTNodeKind::EvalExpression;
        }
    ...
    public:
        ...
        ForIterationMode iterationMode{ForIterationMode::Value};
        ...
        ForStatement(
            ...
            ForIterationMode iterationMode,
    ...
    /// Attribute-scoped declaration groups (`@symbol(...) { ... }`).
    ...
    public:
        ...
        virtual void visitEvalExpression(EvalExpression& expression) = 0;
        virtual void visitDefaultExpression(DefaultExpression& expression) = 0;
        virtual void visitUninitExpression(UninitExpression& expression) = 0;
...
#pragma region ast_cast
    ...
    namespace Internal
    {
        ...
        template <>
        struct ASTNodeKindTraits<EvalExpression> final
        {
            static constexpr ASTNodeKind kind = ASTNodeKind::EvalExpression;
        };

        template <>
        struct ASTNodeKindTraits<DefaultExpression> final
        {
            static constexpr ASTNodeKind kind = ASTNodeKind::DefaultExpression;
        };

        template <>
        struct ASTNodeKindTraits<UninitExpression> final
        {
            static constexpr ASTNodeKind kind = ASTNodeKind::UninitExpression;
        };
