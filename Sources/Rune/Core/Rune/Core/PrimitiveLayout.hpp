...
namespace rune
{
    namespace PrimitiveLayout
    {
        ...
        // Cross-compilation: set the pointer/int size for the target platform.
        // When not overridden (or set to 0) returns the host sizeof(void*).
        // Called by ASTContext when the target platform differs from the host.
        void setTargetPointerSize(usize size);
        usize getTargetPointerSize();
