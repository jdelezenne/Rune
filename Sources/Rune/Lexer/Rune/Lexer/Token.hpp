...
namespace rune::lexer
{
    ...
    enum class TokenType
    {
        ...
        Await,      ///< await keyword.
        Eval,       ///< eval keyword.
        Default,    ///< default expression keyword.
        Uninit,     ///< uninitialized value expression keyword.
        Escape,     ///< escape keyword (marks a reference-taking expression as allowed to leave its scope).
        /// @}
    ...
    /// Full-range magnitude of an Integer token literal. The lexer stores
    /// magnitudes up to int64 max in the int64 alternative and larger ones
    /// (up to uint64 max) in the uint64 alternative; a leading '-' is a unary
    /// operator handled by the parser, so token magnitudes are never negative.
    inline uint64 integerTokenMagnitude(TokenValue const& value)
    {
        if (int64 const* narrow = variantGetIf<int64>(value))
        {
            return static_cast<uint64>(*narrow);
        }

        if (uint64 const* wide = variantGetIf<uint64>(value))
        {
            return *wide;
        }
    ...
    /// Converts a keyword token kind to its source spelling.
    /// @param type Token kind to convert.
    /// @return Keyword spelling, or empty string when the token kind is not a keyword.
    String tokenTypeKeywordString(TokenType type);
