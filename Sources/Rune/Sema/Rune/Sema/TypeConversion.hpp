...
namespace rune::sema
{
    ...
    bool isRuneToCFunctionConversion(ASTContext const& context, TypeID from, TypeID to);
    ...
    /// True for function-pointer types (`(T...) -> R`), which are pointer-sized
    /// and therefore participate in unsafe byte-address casts the same way
    /// references and machine-address integers do.
    bool isFunctionType(ASTContext const& context, TypeID typeId);
