...
namespace rune
{
    ...
    enum class SemanticFunctionCallingConvention : uint8
    {
        Rune,
        C,
    };
    ...
    struct SemanticType final
    {
        ...
        /// Module identifier that owns `nominalDeclId` in a serialized package type.
        /// Live TypeArena entries use the context-local SemanticDeclID and leave this zero.
        IdentifierID nominalModuleNameId{0};
        ...
        bool isAsync{false};
        SemanticFunctionCallingConvention callingConvention{
            SemanticFunctionCallingConvention::Rune};
    ...
    public:
        ...
        /// The universal existential top type `Any`. Cached as a single canonical
        /// type with no name, no arguments, and no nominal decl (like `Never`).
        TypeID internAny();
