...
namespace rune::ril
{
    /// `Any` existential box layout. The box is `typeId` (one 64-bit word) followed
    /// by an inline value buffer of `anyBoxInlineStorageSize` bytes.
    inline constexpr uint32 anyBoxTypeIdSize = 8u;
    inline constexpr uint32 anyBoxInlineStorageSize = 24u;
    inline constexpr uint32 anyBoxTotalSize = anyBoxTypeIdSize + anyBoxInlineStorageSize;
    ...
    struct Type final
    {
        ...
        /// The `Any` existential box: a fixed 32-byte value laid out as
        /// `{ typeId: UInt64, storage: [3 x UInt64] }`. The first word is the
        /// stable introspection identity of the boxed concrete type; the
        /// remaining 24 bytes hold the value inline (types larger than the inline
        /// buffer are not yet boxable). It is a plain aggregate so it flows
        /// through locals, parameters, returns, and collection elements using the
        /// existing aggregate machinery; only `AnyBox`/`AnyExtract` interpret its
        /// bytes specially.
        static Type anyBox();
        /// True for the tagged `Any` existential box aggregate (see `anyBox`).
        bool isAnyBox() const;
    ...
    struct Constant final
    {
        ...
        /// Active case tag for a `Union`-kind constant. Only meaningful when
        /// `type.kind == TypeKind::Union`; mirrors `Value::unionCaseTag`. The constant
        /// evaluator only folds no-payload union constants today, so the payload bytes
        /// are always implicitly zero for a folded union constant.
        ...
        static Constant unionValue(Type valueType, uint32 caseTag)
        {
            return Constant{
                .type = std::move(valueType),
                .unionCaseTag = caseTag,
            };
        }
    ...
    // name. Operations listed here are instead answered by the host runtime
    // itself (the VM interpreter, or the native runtime) from state it already
    // owns - no C symbol and no registration step.
    enum class RuntimeOperation : uint8
    {
        None,
        /// Number of introspection metadata sections of the loaded program.
        IntrospectionDataCount,
        /// Start address of the introspection metadata section at an index.
        IntrospectionDataAddress,
        /// Byte length of the introspection metadata section at an index.
        IntrospectionDataLength,
    };

    RuntimeOperation runtimeOperationForSymbol(String const& symbolName);
    ...
    struct Value final
    {
        ...
        /// Set when the enclosing module declares manifest `includeHeaders`. The C
        /// backend must not emit an `extern` forward declaration for this symbol in
        /// that case — one of the module's included headers already provides it.
        bool foreignHasCIncludeHeader{false};
        ...
        static Value string(String value, uint32 dataSlot = 0)
        {
            return Value{
                .kind = ValueKind::StringLiteral,
                .type = Type::stringType(),
                .stringLiteral = std::move(value),
                .staticDataSlot = dataSlot,
            };
        }

        static Value cString(String value, uint32 dataSlot = 0)
        {
            return Value{
                .kind = ValueKind::StringLiteral,
        ...
        static Value staticDataPointer(Type pointerType, uint32 dataSlot, uint32 dataOffset = 0)
        {
            return Value{
                .kind = ValueKind::StaticDataPointer,
                .type = std::move(pointerType),
                .staticDataSlot = dataSlot,
        ...
        static Value functionReference(uint32 slot, Type valueType)
        {
            return Value{
                .kind = ValueKind::FunctionReference,
                .type = std::move(valueType),
        ...
        // Address of an Option<T>/union value's payload storage, given the
        // union's own address and the payload's byte offset within it (the
        // same offset optionPayloadOffset() computes for the by-value
        // UnionPayload read above) -- lets `optional!.field = value` write
        // directly into the union's storage instead of only being able to
        // read a copy out of it.
        static Value addressOfUnionPayload(uint32 unionAddress, Type resultType, uint32 payloadOffset)
        {
            return Value{
                .kind = ValueKind::AddressOfUnionPayload,
                .type = resultType,
                .operandValue = unionAddress,
            ...
            sema::SemanticDeclID semanticDeclId = sema::invalidSemanticDeclId)
        {
            ...
            value.runtimeOperation = runtimeOperationForSymbol(value.foreignSymbolName);
            return value;
        }
        ...
        static Value witnessTableReference(
            ...
            uint64 concreteTypeId = 0)
        {
            return Value{
                .kind = ValueKind::WitnessTableReference,
                .type = Type::pointer(std::move(witnessTableType)),
                // Stable introspection identity of the concrete type behind this
                // witness table, used by `DynamicTypeMatch` to recover a fat
                // reference's dynamic type at runtime.
                .integerLiteral = concreteTypeId,
                .arguments = std::move(functionSlots),
                .aggregateObjectType = std::move(concreteReceiverType),
            };
        }

        // Tests whether the dynamic type behind a fat reference (`fatReference`,
        // an interface/open-struct reference aggregate) is exactly the type whose
        // stable introspection identity is `targetTypeId`. Yields a `Bool`.
        static Value dynamicTypeMatch(uint32 fatReference, uint64 targetTypeId)
        {
            return Value{
                .kind = ValueKind::DynamicTypeMatch,
                .type = Type::boolType(),
                .integerLiteral = targetTypeId,
                .operandValue = fatReference,
            };
        }

        // Boxes `innerValue` (of arbitrary boxable type) into an `Any` existential.
        // `concreteTypeId` is the stable introspection identity of the inner type,
        // written into the box's `typeId` word so `is`/`as?`/`as!` can recover it.
        // `innerStorageSize` is the inner value's byte size (must be <= the inline
        // buffer; larger types are diagnosed in Sema until heap boxing lands).
        static Value anyBox(uint32 innerValue, uint64 concreteTypeId, uint32 innerStorageSize)
        {
            return Value{
                .kind = ValueKind::AnyBox,
                .type = Type::anyBox(),
                .integerLiteral = concreteTypeId,
                .operandValue = innerValue,
                .staticDataOffset = innerStorageSize,
            };
        }

        // Reads the boxed value out of an `Any` box, reinterpreting the inline
        // storage as `resultType`. This is unconditional: callers must guard it
        // with a `typeId` comparison (`is`) so it only runs on a matching box.
        static Value anyExtract(uint32 boxValue, Type resultType)
        {
            return Value{
                .kind = ValueKind::AnyExtract,
                .type = std::move(resultType),
                .operandValue = boxValue,
            };
        }
            ...
            Array<Type> abiParameterTypes = {})
        {
            return Value{
                ...
                .foreignSymbolName = std::move(symbolName),
                .foreignHasCIncludeHeader = hasCIncludeHeader,
            };
        }
    ...
    struct Statement final
    {
        ...
        static Statement initializeGlobal(uint32 slot, uint32 storedValue)
        {
            return Statement{
                .kind = StatementKind::InitializeGlobal,
                .globalSlot = slot,
                .value = storedValue,
            };
        }
