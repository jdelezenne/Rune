...
Rune is a statically typed programming language for systems and application development, implemented in C++23. It is cross-platform on macOS and Windows, and supports three backends: LLVM (native executable), C23 (generated code), and VM (custom Bytecode and Virtual Machine).
...
- **Native C interop** — direct symbol/header/library integration through `@symbol`, `@include`, and `@link`.
...
$RUNE build Examples/Output/Hello.rune --executable --name Hello --backend c -o /tmp/Hello/

# Build a native executable with the LLVM backend
$RUNE build Examples/Output/Hello.rune --executable --name Hello --backend llvm -o /tmp/Hello/
```
...
| `Examples/Output/` | Programs with console output |
| `Examples/Modules/SDL3/` | SDL3 bindings written in Rune |
...
```bash
cmake --build Build/ninja-debug --target RuneFeatureTests RuneExampleSmokeTests
ctest --preset ninja-debug --output-on-failure
```

## Distributions

The build defines two install-backed staging targets.

Create a runnable Rune distribution with the compiler executable and builtin
modules:

```bash
cmake --build Build/ninja-debug --target Distribution
```

This stages:

```text
Build/ninja-debug/Distribution/rune
Build/ninja-debug/Distribution/modules/Rune.runemodule
Build/ninja-debug/Distribution/modules/Standard.runemodule
Build/ninja-debug/Distribution/modules/LibC.runemodule
```

Create a development distribution with public repository material:

```bash
cmake --build Build/ninja-debug --target DevelopmentDistribution
```

This stages C/C++ headers, `Code/Modules`, examples, user/developer
documentation, `README.md`, and `LICENSE` under:

```text
Build/ninja-debug/DevelopmentDistribution/
Build/ninja-debug/DevelopmentDistribution/Sources/
Build/ninja-debug/DevelopmentDistribution/Examples/
Build/ninja-debug/DevelopmentDistribution/Documentation/
```

Agent documentation is intentionally not included in the development
distribution.
