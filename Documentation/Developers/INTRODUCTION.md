...
| Platform | `Platform::HostPlatform`, `windows` / `darwin` / `linux` |
...
The existing `Introspection.rune` source surface, the compiler metadata that feeds it, and the C/C++ runtime or binding implementation behind it require a **full rewrite** as part of the compiler flow redesign. This is not a compatibility migration of the current introspection API. The new implementation will consume an optional introspection section in `.runemodule` through typed metadata; it will not recover reflection data from bytecode, RIL, debug metadata, source parsing, or backend-specific layouts.
...
```bash
cmake --preset ninja-debug
cmake --build Build/ninja-debug --target RuneSemaFeatureTests RuneCoreFeatureTests
