...
For typed initialization expressions:

- `default` creates the default value for the contextual type.
- `uninit` (unsafe mode only) creates uninitialized storage for the contextual
    type and must be fully assigned before any read.

`uninit` is accepted only for primitive types and for structs that do not
declare explicit `init` or `final`.
