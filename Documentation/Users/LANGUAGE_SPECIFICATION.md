...
```text
alias and as async await break by catch const continue default defer dispose
distinct else enum escape eval extends extension false final for func if
implements import in init interface internal is let literal loop make
match mutating nil not open operator or override private public return
self Self static struct super throw throws true try type union var while
uninit
```
...
Rune supports two special typed value expressions:

- `default`: produces the default value of the contextual type.
- `uninit`: produces an uninitialized value of the contextual type.

`uninit` is restricted to unsafe module mode and requires a contextual type.
It is allowed for primitive types and for structs that do not declare explicit
`init` or `final` methods.
...
The native library to link and the C header a binding requires are not
per-declaration attributes — they are manifest settings (`linkLibraries`,
`includeHeaders`; see [Module manifests](MODULE_MANIFEST.md)), or `-l`/
`--include-header` CLI flags for source builds without a manifest.
...
`Any` is a built-in universal existential type — every value implicitly widens
("boxes") into it, and no up-front conformance is required:

```rune
let a: Any = 42                       // boxes an Int
let values: Array<Any> = [1, true, 3.0]   // heterogeneous collection
func log(value: Any) { /* ... */ }
```

Widening is implicit; narrowing is not. Recover the boxed value with a runtime
type test or cast:

```rune
let a: Any = 42
if a is Int {
    let n: Int  = a as! Int   // traps on a type mismatch
    let m: Int? = a as? Int   // nil on a type mismatch
}
```

`a is T` yields `Bool`, `a as? T` yields `T?`, and `a as! T` yields `T` (trapping
on a mismatch). Plain `as` cannot narrow an `Any`. The requested type is matched
exactly against the boxed value's dynamic type.

An `Any` box holds its value inline and carries the value's stable type identity
(the same value `#typeIdOf<T>()` returns), so `a`'s dynamic type composes with the
introspection `TypeRegistry`. Values that own heap storage (`String`, `Array`, …),
that have a `final` finalizer, or that are too large for the inline buffer cannot
be boxed by value yet — box a reference to them instead (`let a: Any = &value`).
...
Borrowing a fixed inline array preserves the array type and its length. For
example, `*values` has type `*[T, N]`; it does not implicitly decay to `*T`.
Borrow an individual element explicitly (`*values[0]`) when an element
...
The expression `default` produces the contextual type's default value. It is
valid only where the expected type is known, such as typed binding
initializers, global initializers, return expressions, call arguments, default
parameter values, and struct field default values:

```rune
const zero: Int = default
let absent: Int? = default

struct Options {
    var retries: Int = default
}
...
Without a contextual type, `default` is ill-formed. As a `const` initializer,
`default` is a compile-time constant only for primitive and string defaults.

`eval` may appear in a `const` initializer to execute ordinary Rune code during
compilation:
...
const mask = eval buildMask()
```

The evaluated value is embedded as readonly module data. `eval` is not valid in
`let` initializers, `var` initializers, return expressions, call arguments, or
other runtime expression positions.
...
Catch clauses support a bound error, a pattern, or a catch-all block. A `do`
...
`as?` / `as!` operate on a **reference** (polymorphism lives behind references),
and the result type is exactly the target written — the same rule as primitives
(`x as? Int8` has type `Int8?`). A downcast therefore names a reference target:

```rune
let s: *Shape = *circle
let maybe: (*Circle)? = s as? *Circle   // reference to the same object, no copy
let forced: *Circle   = s as! *Circle   // traps on a mismatch
```

`is` tests the referent's dynamic type and accepts either spelling (`s is Circle`
or `s is *Circle`). Using `as?` / `as!` on a struct *value*, or with a value
target on a reference source, is rejected with guidance — a value's type is
statically known, so a runtime-checked cast is never needed.
...
Lambdas may capture values from their surrounding lexical scope:
...
Captured locals are resolved by their semantic declaration, so shadowed names
refer to the declaration that is actually in scope at the capture site.
...
spelled with `const func`; use `eval` inside a const initializer to execute an
ordinary function during compilation.

`eval` type-checks its operand as ordinary Rune code. There is no separate
compile-time function kind and no reduced eval-only language subset: if the code
can be lowered to the VM for normal execution, the compiler uses that same VM
execution path for compile-time evaluation.

```rune
func makeValues() -> Array<Int> {
    var values: Array<Int> = [1, 2, 3]
    values.append(8)
    return values
}

const values = eval makeValues()
```

The result of an `eval` expression is materialized into the module's readonly
static data before final backend emission. LLVM, C, and VM backends consume that
ordinary static data; they do not replay the eval call graph or implement eval
themselves.

`eval` requires a compiler build with VM backend support. If the VM backend is
not enabled in the compiler, use of `eval` is rejected.
...
```rune
static operator +(lhs: Vector, rhs: Vector) -> Vector { ... }
static operator !(value: Flag) -> Bool { ... }       // prefix
operator ()! -> T { ... }                            // postfix
operator [](index: Int) -> T { ... }
operator [](row: Int, column: Int) -> T { ... }
operator []=(index: Int, value: T)
```

Instance operators cannot be `static`. Subscript assignment `[]=` requires at
least one parameter. Subscript operators may declare multiple parameters, and a
subscript expression passes the bracket arguments to that operator in order:
`grid[row, column]`.
...
Non-mutating methods borrow `self` immutably: they may read fields but cannot
assign to fields or to `self`. Mutating methods borrow `self` mutably and may
modify fields directly or through `self`. Receiver borrowing is implicit, so an
instance method does not copy its receiver regardless of the receiver's value
size:
...
struct User implements Hashable {
    static operator ==(lhs: User, rhs: User) -> Bool { ... }
    func hash() -> Int64 { ... }
}
```

Interfaces can require operators. `Equatable` requires static `==`,
`Hashable` extends `Equatable` with `hash()`, and `Comparable` extends
`Equatable` with static `<=>` returning `Ordering`.
...
Bare `try` is rejected outside a matching throwing function. Use `do { } catch`
...
`do` block. The compiler's internal result value for a throwing initializer is
not a user-storable value.

### Do-Catch

`do { } catch { }` handles errors with catch clauses. Multiple catch clauses are evaluated in order:
...
A parameter declared as `T` has value semantics; passing an aggregate to it
copies the aggregate. Declare observational APIs as `*T`, and use `&T` only
when the callee may mutate the referent. Aggregate size does not implicitly
change a value parameter into a reference.
...
|---|---|---|
| `make` | `&T` | Traps, or is catchable inside `do` |
| `make!` | `&T` | Traps |
| `make?` | `Option<&T>` | Returns `nil` |

Plain `make` (without `!` or `?`) is allowed only in `throws` functions or
inside a `do` block. In non-throwing functions, use `make!` or `make?`:
...
Inside a `do` block, plain `make` turns allocation failure into a catchable
error:
...
Rune functions can be bound to C symbols using the `@symbol(...)` attribute,
naming the exact C symbol to bind. The function body is omitted; Rune calls
...
The native library to link and the C header that declares the symbol are not
part of the attribute — they are settings on the module's manifest
(`linkLibraries`, `includeHeaders`), or `-l`/`--include-header` CLI flags for
source builds without a manifest. See [Module manifests](MODULE_MANIFEST.md).

Multiple bindings can be grouped in a braced block to share attributes:
...
A byte-address value (`&T`, `*T`, or a machine-address integer) can be cast
to a function-pointer type (`(Params...) -> Return`) with `as`, the same way
it can be cast to another reference or integer type. This is how a symbol
resolved dynamically at runtime - `dlsym`, `GetProcAddress`,
`SDL_GL_GetProcAddress`, and similar loader APIs all return an opaque byte
reference - becomes a value you can actually call:

```rune
let symbol = LibC.dlsym(handle, "getpid")
let getpid = symbol! as () -> Int32
let pid = getpid()
```

The source is almost always an `Optional` reference (`&Byte?`); force-unwrap
it (`symbol!`) before the cast; casting the optional itself is not supported.
The reverse direction also works (`function as &Byte`) if you need the raw
address back out.

Parameter and return types in the cast's function signature use the same
ABI-aware lowering as an `@symbol`-declared function: an `Optional` raw
pointer (`*T?`) lowers to a plain nullable C pointer (not a tagged union),
so `nil` is safe to pass through such a parameter. `&x` also builds a
genuine second level of indirection when `x` is itself reference/pointer
typed (for a local `var x: *T`, taking `&x` yields a real `*(*T)`), so a
`T**`-shaped argument (e.g. `glShaderSource`'s `strings` parameter) can be
built directly: `var pointer: *T = value` then `&pointer`.
...
`Thread(body:)` starts a joinable thread running Rune bytecode. The body is a
synchronous `() -> Void` value. It may be a top-level function, a noncapturing
lambda, or a closure that captures immutable value locals:
...
Captured values are copied into an owned thread environment before the OS thread
starts. The new thread does not keep a pointer to the caller's stack frame.

For now, thread closures reject mutable local captures, reference captures, and
captured function or closure values. Use module-level state protected by
`Mutex`/`ReadWriteMutex`, or pass immutable value snapshots into the thread
closure.

`Thread` also exposes current-thread scheduler helpers:

```rune
let duration = Duration.fromMilliseconds(value: 16)
Thread.sleep(duration: duration)
Thread.sleepUntil(deadline: Instant.now() + duration)
Thread.yield()
...
`Duration` is elapsed time. `Instant` is monotonic and should be used for
measurement and sleep deadlines. `SystemTime` is wall-clock time and may move
backward or forward if the operating system clock is adjusted. Civil `Date` and
`Time` are distinct calendar/time-of-day values, not scheduler primitives.

`join()` waits for completion; `detach()` releases the handle without waiting.

Use `Standard.Mutex`, `Standard.ReadWriteMutex`, or future atomics to
synchronize access to shared module `var` globals or other data reachable from
multiple threads.
...
| Shared globals | Module-level `var` is process-wide; protect with `Mutex`/`ReadWriteMutex` when mutated concurrently |
...
```rune
#embedSource "Shared.rune"

const shader: String = #embedString("Shaders/Main.wgsl")
const logo: Array<Byte> = #embedBytes("Assets/Logo.bin")
```

`#embedSource` copies another Rune source file into the current source file before lexing and parsing. The path must be quoted and is resolved relative to the file that contains the directive. Embedded source files may contain other `#embedSource` directives, but recursive source embedding cycles are rejected.

`#embedString` and `#embedBytes` read a file at compile time and embed its contents in the program. Their path argument must be a string literal and is resolved relative to the source file that contains the macro call. `#embedString` behaves like a string literal, so any type eligible for string-literal initialization may be used as the contextual type. `#embedBytes` behaves like an array literal of `Byte` values, so any type eligible for array-literal initialization from bytes may be used as the contextual type.
...
These macros surface type information at compile time. Both accept either a single type argument or a single expression argument; generic parameter types are rejected because they have no concrete identity at the use site.

```rune
let id = #typeIdOf<Point>()              // UInt64 stable type identity
let same = #typeIdOf(point)              // expression form: static type of the operand
```

The expression form uses the operand's static type. The operand is never evaluated — the macro lowers to a compile-time constant. Untyped literals materialize to their default types first (`#typeIdOf(1)` is the identity of `Int`).

`#typeIdOf<T>()` returns a stable `UInt64` identity for `T`, hashed from the canonical module-qualified spelling of the type (for example `Main.Point` or `Rune.Array<Rune.Int>`). The value is deterministic across builds and unique per type spelling, which makes it suitable for keys in registries, serialization tags, and cross-module type identification. The same identity appears in the `.runemodule` introspection section.

```rune
import Introspection.*

let info: TypeInfo = #typeInfoOf<Point>()
let name = info.qualifiedName            // "Main.Point"
let kind = info.kind                     // TypeKind.structure
let x = info.field(name: "x")            // FieldInfo? with type, offset, size
```

`#typeInfoOf<T>()` returns an `Introspection.TypeInfo` value describing `T`: identity (`typeId`, matching `#typeIdOf`), names (`name`, `qualifiedName`), classification (`kind: TypeKind`), and layout (`size`, `alignment`, `stride`). Kind-specific metadata is included where it applies:

- **Structs** — `fields: Array<FieldInfo>` with each stored field's name, type identity, qualified type name, offset, and size.
- **Enums and flags** — `cases: Array<CaseInfo>` with each case's name and raw value, plus `rawTypeId` identifying the raw type (`enum Mode: Int32` reports the identity of `Rune.Int32`).
- **Unions** — `cases` with each case's name and declaration index as the value.
- **Distinct types** — `rawTypeId` identifying the base type.

`TypeInfo` offers lookups over this metadata: `field(name:)`, `caseNamed(name:)`, `caseName(value:)`, `hasCase(name:)`, and classification helpers such as `isStruct()`, `isEnum()`, and `isUnion()`. The macro requires `import Introspection.*` because the result type is defined by the `Introspection` standard module. The construction is fully evaluated at compile time.

The program's registry is available through `typeRegistry()`:

```rune
let registry = typeRegistry()
let point = registry.typeInfo(name: "Main.Point")       // current module
let imported = registry.typeInfo(name: "Physics.Body")  // imported module

let id = TypeRegistry.typeId(name: "Main.Point")        // same value as #typeIdOf<Point>()
let byId = registry.typeInfo(typeId: id)                // TypeInfo?
```

Type metadata travels in the compiled artifact itself, the way reflection
metadata lives in a managed assembly: every module persists an introspection
metadata section — `.runemodule` files carry a record for each of their type
declarations, and a program artifact carries the merged records of the
program (the current module plus every imported module, de-duplicated by
identity). The Rune runtime initializes the type registry directly from this
persisted metadata; there is no registration step, manual or otherwise, and
no build flag — the metadata is always present.

`TypeRegistry.typeId(name:)` hashes a canonical qualified type name at
runtime with the same FNV-1a hash the compiler uses, so string-derived
identities agree with `#typeIdOf`. The section encoding is one versioned
little-endian format (`RINT` magic, format version, one record per type)
shared by `.runemodule` sections, program artifacts, and the runtime decoder.
