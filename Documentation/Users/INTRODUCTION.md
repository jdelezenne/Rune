...
Rune runs on Windows, macOS, and Linux through its LLVM, C23, and VM
backends. The compiler, VM, and standard library can also be built as a
WebAssembly application, allowing Rune source to be compiled and executed
entirely in the browser; the [Rune Playground](../../Playground/index.html)
is the reference integration.

The repository includes reusable bindings and complete examples for:

- [SDL3](../../Examples/Modules/SDL3/) — windows, input, rendering, audio,
  gamepads, and other native application facilities.
- [WebGPU](../../Examples/Modules/WebGPU/) — browser/WASM graphics and compute,
  with examples ranging from textured rendering to particle simulation and
  complete applications.

Rune also provides compile-time and runtime introspection. `#typeIdOf<T>()`
produces a stable type identity, `#typeInfoOf<T>()` exposes names, layout,
fields, and cases through the `Introspection` module, and `typeRegistry()`
queries metadata collected from the current program and its imported modules.
See [Introspection macros](LANGUAGE_SPECIFICATION.md#introspection-macros) for
the complete API.
...
`alias`, `and`, `as`, `break`, `catch`, `const`, `continue`, `default`, `defer`,
...
Use `default` when a value should be initialized from its contextual type's
default value. It requires a contextual type, so it is valid in typed
declarations, return expressions, function arguments, function parameter
defaults, struct field defaults, and global initializers:
...
func clamp(value: Int, min: Int = default) -> Int {
    if value < min { return min }
    return value
}
```

Use `eval` inside a `const` initializer to execute an ordinary Rune expression
during compilation. The evaluated result is stored as readonly module data, so
the final backend sees a normal constant value rather than an eval operation.

```rune
func buildDefaults() -> Array<Int> {
    var values: Array<Int> = [10, 20]
    values.append(30)
    return values
}

const DEFAULTS = eval buildDefaults()
```

`eval` is only valid in `const` initializers. It is not a macro and is written
without `#`; compiler builds without the VM backend reject `eval`.
...
| `var x: T = value` or `let x: T = value` | **Optional** — default used when omitted; `value` may be `default` |
...
struct Vector {
    ...
    static operator ==(lhs: Vector, rhs: Vector) -> Bool {
        return lhs.x == rhs.x and lhs.y == rhs.y
    }
}
```

Subscript operators are instance members. Symbolic unary and binary operators
are static members.
...
Handle throwing code with `do` / `catch` blocks:
